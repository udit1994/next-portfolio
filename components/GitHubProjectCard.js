 ...
 */
function GitHubProjectCardStars({ repository: path, ...props }) {
 ...
 */
function GitHubProjectCardDate({ repository: path, ...props }) {
  ...
  useEffect(() => {
    const get = async () => {
      ...
      setCreatedDate(repository?.created);
    };

    get();
  }, []);

  return <span {...props}>{createdDate ? <RelativeDate date={createdDate} /> : '—'}</span>;
}
 ...
 */
export function GitHubProjectCard({ children, repository, ...props }) {
  return (
    <ProjectCard {...props}>
      <div className="mb-8 flex items-center text-zinc-400">
        <small className="text-2xs flex items-center gap-4 font-semibold uppercase leading-tight tracking-widest">
